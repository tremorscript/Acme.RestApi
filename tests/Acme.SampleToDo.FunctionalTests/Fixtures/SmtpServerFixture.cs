namespace Acme.SampleToDo.FunctionalTests.Fixtures;
  ...
  public void EnsureContainerIsRunning()
  {
    if (_container == null)
    {
      throw new InvalidOperationException(
        "SMTP server container was not created. Ensure Docker is running and the container image is correct.");
    }

    if (_container.State != TestcontainersStates.Running)
    {
      throw new InvalidOperationException(
        "The SMTP server container is not running. Please verify that the SMTP server image is correctly configured and that Docker is functioning properly.");
    }
  }
}
